...
class DeviceGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Device entity")
        ...
        .repeat(2) {
            ...
            .get("/pondnotes/api/devices")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new device")
            .post("/pondnotes/api/devices")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "deviceName":"SAMPLE_TEXT", "deviceType":null, "description":"SAMPLE_TEXT", "timestamp":"0", "userId":"0"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_device_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created device")
                .get("/pondnotes${new_device_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created device")
            .delete("/pondnotes${new_device_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
