...
class LocationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Location entity")
        ...
        .repeat(2) {
            ...
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "streetAddress":"SAMPLE_TEXT", "postalCode":"SAMPLE_TEXT", "city":"SAMPLE_TEXT", "county":"SAMPLE_TEXT", "timestamp":"0", "userId":"0"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_location_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created location")
                .get("/pondnotes${new_location_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created location")
            .delete("/pondnotes${new_location_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
